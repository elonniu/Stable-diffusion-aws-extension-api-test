from __future__ import print_function

import logging
import time
from datetime import datetime
from datetime import timedelta

import stable_diffusion_aws_extension_api_test.config as config
from stable_diffusion_aws_extension_api_test.utils.api import Api
from stable_diffusion_aws_extension_api_test.utils.helper import delete_sagemaker_endpoint

logger = logging.getLogger(__name__)

endpoint_data = {}


class TestEndpointE2E:

    def setup_class(self):
        self.api = Api(config)

    @classmethod
    def teardown_class(cls):
        pass

    def test_1_endpoints_delete(self):
        delete_sagemaker_endpoint(self.api)

    def test_2_no_available_endpoint(self):
        headers = {
            "x-api-key": config.api_key,
            "Authorization": config.bearer_token
        }

        params = {
            "username": config.username
        }

        list = self.api.list_endpoints(headers=headers, params=params)

        if 'endpoints' in list and len(list["endpoints"]) > 0:
            return

        headers = {
            "x-api-key": config.api_key,
            "Authorization": config.bearer_token
        }

        data = {
            "user_id": config.username,
            "task_type": "txt2img",
            "models": {
                "Stable-diffusion": [config.default_model_id],
                "embeddings": []
            },
            "filters": {
                "createAt": datetime.now().timestamp(),
                "creator": "sd-webui"
            }
        }

        resp = self.api.create_inference(headers=headers, data=data)
        assert resp.status_code == 200
        assert resp.json()["statusCode"] == 500
        assert resp.json()["errMsg"] == 'no available Endpoints for user "admin"'

    def test_3_endpoints_post(self):
        headers = {
            "x-api-key": config.api_key,
            "Authorization": config.bearer_token
        }

        data = {
            "endpoint_name": config.endpoint_name,
            "instance_type": config.instance_type,
            "initial_instance_count": "1",
            "autoscaling_enabled": False,
            "assign_to_roles": ["IT Operator"],
            "creator": config.username
        }

        resp = self.api.create_endpoint(headers=headers, data=data)
        assert resp.status_code == 200
        assert resp.json()["data"]["endpoint_status"] == "Creating"

    def test_4_endpoints_post_exists(self):
        headers = {
            "x-api-key": config.api_key,
            "Authorization": config.bearer_token
        }

        data = {
            "endpoint_name": config.endpoint_name,
            "instance_type": config.instance_type,
            "initial_instance_count": config.initial_instance_count,
            "autoscaling_enabled": False,
            "assign_to_roles": ["Designer"],
            "creator": config.username
        }

        resp = self.api.create_endpoint(headers=headers, data=data)
        assert resp.status_code == 200
        assert "Cannot create already existing endpoint" in resp.json()["message"]

    def test_5_endpoints_get_status(self):
        headers = {
            "x-api-key": config.api_key,
            "Authorization": config.bearer_token
        }

        params = {
            "username": config.username
        }

        resp = self.api.list_endpoints(headers=headers, params=params)
        assert resp.status_code == 200
        assert len(resp.json()["endpoints"]) >= 0
        assert f"infer-endpoint-{config.endpoint_name}" in [endpoint["endpoint_name"] for endpoint in
                                                            resp.json()["endpoints"]]

        timeout = datetime.now() + timedelta(minutes=30)

        while datetime.now() < timeout:
            result = self.endpoints_wait_for_in_service()
            if result:
                break
            time.sleep(50)
        else:
            raise Exception("Function execution timed out after 30 minutes.")

    def endpoints_wait_for_in_service(self):
        headers = {
            "x-api-key": config.api_key,
            "Authorization": config.bearer_token
        }

        params = {
            "username": config.username
        }

        resp = self.api.list_endpoints(headers=headers, params=params)
        assert resp.status_code == 200
        for endpoint in resp.json()["endpoints"]:
            if endpoint["endpoint_name"] == f"infer-endpoint-{config.endpoint_name}":
                if endpoint["endpoint_status"] != "InService":
                    logger.info(
                        f"{endpoint['endpoint_name']} is {endpoint['endpoint_status']}")
                    return False
                else:
                    return True

        return False

    # not support a same role create more than one endpoint
    def test_6_endpoints_create_role_limit(self):
        headers = {
            "x-api-key": config.api_key,
            "Authorization": config.bearer_token
        }

        data = {
            "endpoint_name": config.endpoint_name,
            "instance_type": config.instance_type,
            "initial_instance_count": "1",
            "autoscaling_enabled": False,
            "assign_to_roles": ["IT Operator"],
            "creator": config.username
        }

        resp = self.api.create_endpoint(headers=headers, data=data)
        assert resp.status_code == 200
        assert resp.json()["statusCode"] == 400
        assert 'not allow to have another one' in resp.json()["message"]
